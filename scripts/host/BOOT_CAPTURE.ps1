...
$sftool = Get-ChildItem (Join-Path $RepoRoot "tools\sftool") -Recurse -Filter sftool.exe -ErrorAction SilentlyContinue | Select-Object -First 1 -ExpandProperty FullName
$bin = Join-Path $ArtifactsDir ($Bin -replace '^artifacts\\','')
$out = Join-Path $ArtifactsDir "boot_capture_latest.txt"
